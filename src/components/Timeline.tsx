...
const Timeline = () => {
  ...
  return <section id="timeline" className="py-20 bg-muted/30">
      <div className="container mx-auto px-6">
        ...
        <div className="max-w-4xl mx-auto">
          <div className="relative">
            ...
            {timelineItems.map((item, index) => <motion.div key={item.id} initial={{
            opacity: 0,
            x: -30
          }} whileInView={{
            opacity: 1,
            ...
            delay: index * 0.2
                ...
                <div className={`relative z-10 w-16 h-16 rounded-2xl border-2 flex items-center justify-center ${getBgColor(item.type, item.isOngoing)}`}>
                  ...
                  {item.isOngoing && <div className="absolute -top-1 -right-1 w-4 h-4 bg-primary rounded-full animate-pulse">
                ...
                <div className="flex-1 card-gradient card-hover p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {item.skills.map(skill => <span key={skill} className="px-3 py-1 text-xs font-medium bg-primary/10 text-primary rounded-full border border-primary/20">
                        {skill}
                      </span>)}
                  </div>
                </div>
